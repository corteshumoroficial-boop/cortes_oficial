...
export interface ViralClip {
  id: string; // 🎬 ID exclusivo do corte
  title: string;
  score: number;
  ...
  properties: {
    clips: {
      ...
      items: {
        type: "object",
        properties: {
          title: { type: "string", description: "Título viral curto e magnético EM PORTUGUÊS (max 80 chars)" },
          ...
          hookQuote: { type: "string", description: "A frase exata do hook falada no vídeo em português (primeiros 3s)" },
          triggers: {
            type: "array",
            items: {
              type: "string",
          ...
          justification: { type: "string", description: "Justificativa EM PORTUGUÊS de por que funciona como Short independente (1-2 frases)" },
        },
        required: [
          "title",
          "score",
...
function isHallucinated(text: string, transcript: string): boolean {
  if (!text || !transcript) return true;
  
  // Normalizar: remover pontuação, converter para minúsculas e dividir em palavras com mais de 2 letras
  ...
  // Se mais de 45% das palavras do texto gerado não estão na transcrição original, consideramos alucinação
  return (missingCount / textWords.length) > 0.45;
}

function isTranscriptPortuguese(transcript: string): boolean {
...
// ⏱️ Helpers para parsing e extração determinística de trechos
...
function extractTranscriptExcerpt(transcript: string, startTs: string, endTs: string): string {
  ...
  for (const line of lines) {
    ...
    // Suporta formatos de timestamp na transcrição: [0:00], [00:00], [00:00:00]
...
function scoreSegment(text: string): { score: number; triggers: ClipTrigger[] } {
  const lower = text.toLowerCase();
  let score = 50; // Pontuação base
  const triggers: ClipTrigger[] = [];

  // Palavras-chave de alto impacto/engajamento
  ...
  for (const kw of hookKeywords) {
    if (lower.includes(kw)) { hasHook = true; break; }
  }
  for (const kw of highValueKeywords) {
    if (lower.includes(kw)) { hasHighValue = true; break; }
  }
  for (const kw of emotionalKeywords) {
    if (lower.includes(kw)) { hasEmotional = true; break; }
  }
  for (const kw of controversyKeywords) {
    if (lower.includes(kw)) { hasControversy = true; break; }
  }
  for (const kw of humorKeywords) {
    if (lower.includes(kw)) { hasHumor = true; break; }
  }
  ...
  // Limitar pontuação a 98 para deixar espaço de refinamento
...
function buildEngagingTitleAndHook(text: string, triggers: ClipTrigger[], startTimestamp: string): { title: string; hookQuote: string } {
...
function quickAnalyzeTranscript(transcript: string, videoTitle: string): { clips: ViralClip[] } {
  ...
  // Primeiro passo: detectar se o texto possui timestamps formatados
  ...
  // Segundo passo: parsear os trechos/segmentos de forma cronológica
  ...
  for (const line of lines) {
    ...
    if (tsMatch) {
      ...
      if (hasAnyTimestamps) {
        // Se o vídeo possui timestamps, estimamos o tempo com base no tamanho das palavras do trecho anterior
        // Usamos uma média realista de fala (150 palavras por minuto, ~0.4s por palavra)
        const prevWords = segments.length > 0 ? segments[segments.length - 1].wordCount : 0;
        currentSec += prevWords * 0.4;
      } else {
        // Sem timestamps, estimativa pura do tempo corrido acumulado
  ...
  // Fallback absoluto se a transcrição estiver vazia ou mal formatada
  ...
  // Terceiro passo: Calcular scores e triggers para cada segmento
  const scoredSegments = segments.map((seg, idx) => {
    const { score, triggers } = scoreSegment(seg.text);
    return {
      segment: seg,
      index: idx,
      score,
  ...
  const selected: typeof scoredSegments[number][] = [];

  // Quarto passo: Selecionar 5 trechos cronológicos distanciados (Zonas)
  // Dividimos o vídeo em 5 partes para garantir que os cortes não se repitam ou se sobreponham!
  if (scoredSegments.length <= 5) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (zoneItems.length > 0) {
        // Encontrar o segmento com maior potencial viral da zona
        let best = zoneItems[0];
        for (const item of zoneItems) {
          if (item.score > best.score) {
            best = item;
          } else if (item.score === best.score && item.segment.wordCount > best.segment.wordCount) {
            best = item; // Prefere frases mais longas para dar mais substância ao corte
          }
        }
        selected.push(best);
      }
  ...
  // Garantir que temos exatamente 5 clipes (preenchendo se houver poucos segmentos)
  ...
  // Ordenar de volta cronologicamente para montar os dados dos cortes
  selected.sort((a, b) => a.segment.startSec - b.segment.startSec);

  // Quinto passo: Gerar os clipes expandindo o tempo do trecho selecionado para atingir entre 30 e 45 segundos
  const candidates: ViralClip[] = selected.slice(0, 5).map((item) => {
    const seg = item.segment;
    const triggers = item.triggers;
    const score = item.score;
    const id = `clip_${Math.random().toString(36).substring(2, 9)}_${Date.now()}`;

    // Acumula segmentos subsequentes até dar pelo menos 30 segundos e no máximo 45 segundos
  ...
  // Retorna os clipes ordenados pelo score de viralidade (maior primeiro)
  return { clips: candidates.sort((a, b) => b.score - a.score) };
}

function normalizeClipRange(clip: ViralClip): { start: number; end: number } {
...
function clipLooksRepeated(clip: ViralClip, other: ViralClip): boolean {
  ...
  const normalizeText = (text: string) =>
    text.toLowerCase().replace(/[\p{P}\p{S}]+/gu, " ").replace(/\s+/g, " ").trim();
...
function ensureDistinctClipSet(clips: ViralClip[], transcript: string, videoTitle: string): ViralClip[] {
  const limited = clips.slice(0, 5);
  if (limited.length <= 1) {
    return limited;
  }

  const fallback = quickAnalyzeTranscript(transcript, videoTitle).clips;
...
/**
 * Tenta recuperar clips de um JSON truncado gerado pelo Ollama.
 * Modelos pequenos (1b, 3b) frequentemente cortam a resposta no meio do JSON.
 * Estratégia: encontrar todos os objetos de clip completos (com todas as chaves obrigatórias)
 * e construir um objeto válido com eles.
 */
function repairTruncatedOllamaJson(rawJson: string): { clips: any[] } {
  console.warn("⚠️ JSON truncado detectado. Tentando recuperar clips completos...");
  
  // Encontrar a abertura do array clips
  ...
  // Extrair o conteúdo do array e encontrar objetos completos
    ...
    if (char === "{") {
      ...
      if (depth === 0 && objectStart !== -1) {
        ...
        try {
          ...
          // Verificar se tem os campos obrigatórios mínimos
...
export const analyzeTranscript = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ClipAnalysisResult> => {
    ...
    const aiConfig = isLovableValid
      ...
      const shouldGenerateThumbnails = data.generateThumbnails !== false;

      // Fast analyze mode: when enabled or when using a small local Ollama model,
      // return quick heuristic results to avoid long waits.
      try {
        ...
        if (fastModeEnv || useFastByDefault) {
          ...
          if (shouldGenerateThumbnails && data.videoPath) {
            ...
            const TRIGGER_BG_MAP: Record<string, "dark_gradient" | "vibrant_gradient" | "city_night" | "abstract"> = {
            ...
            if (isYouTube) {
              try {
                ...
                const resolved = execSync(ytdlpCmd, { encoding: "utf-8", timeout: 25000 }).trim().split("\n")[0];
            ...
            const quickClipsWithThumbs = [] as typeof quick.clips;
            for (let index = 0; index < quick.clips.length; index++) {
              ...
              try {
                ...
                const selectedTrigger = allTriggerTypes.includes(clipRealTrigger as any)
                  ? (clipRealTrigger as typeof allTriggerTypes[number])
                  : allTriggerTypes[index % allTriggerTypes.length];
            ...
            return { clips: ensureDistinctClipSet(quickClipsWithThumbs, data.transcript, data.videoTitle || "") };
          }

          return { clips: ensureDistinctClipSet(quick.clips, data.transcript, data.videoTitle || "") };
        }
      } catch (err) {
    ...
    // 🧠 Recuperar exemplos de feedback positivos (rating = 1) no Supabase para Few-Shot Learning
    let positiveExamplesPrompt = "";
    try {
      const admin = workerSupabase as any;
      const { data: feedbackExamples } = await admin
        .from("clip_feedback")
        .select("*")
        .eq("rating", 1)
      ...
      if (feedbackExamples && feedbackExamples.length > 0) {
        positiveExamplesPrompt = `\n\nEXEMPLOS DE REFERÊNCIA (Esses clipes foram altamente aprovados/corrigidos pelo usuário no passado. Siga rigorosamente este estilo de título e hook):\n` +
          feedbackExamples.map((ex: any, idx: number) => `
Exemplo ${idx + 1}:
...
Extraia os 5 melhores clipes virais (30-60s) com timestamps reais, score de viralidade e justificativa.`;
    ...
    try {
      if (aiConfig.provider === "ollama") {
        ...
        // Ollama doesn't support tool_choice, so we'll use a structured prompt and JSON mode
        let response: Response;
        try {
          const controller = new AbortController();
          const timeout = setTimeout(() => controller.abort(), 600000); // 10 minutos de timeout para Ollama local
          response = await fetch(aiConfig.endpoint, {
            ...
            body: JSON.stringify({
              ...
              options: {
                num_predict: 4096,   // ⚡ Garante saída completa — evita JSON truncado
                temperature: 0.3,    // Mais determinístico para JSON estruturado
              },
              messages: [
                { role: "system", content: SYSTEM_PROMPT + (positiveExamplesPrompt ? `\n\n${positiveExamplesPrompt}` : "") },
                { 
                  role: "user", 
          ...
          const errorMsg = isTimeout
            ? `O processamento do Ollama excedeu o tempo limite (10 minutos).\n\nComo o Ollama roda localmente no seu computador, analisar transcrições longas sem uma placa de vídeo dedicada (GPU) ativa pode ser extremamente lento e travar a execução.\n\nSolução recomendada:\nConfigure uma chave de IA no arquivo .env (ex: OPENAI_API_KEY ou LOVABLE_API_KEY) para obter os resultados em poucos segundos de forma estável.`
        ...
        if (!response.ok) {
          const statusText = `Ollama retornou status ${response.status}`;
        ...
        if (!jsonMatch) {
          return { clips: [], error: "Ollama retornou resposta inválida (JSON não encontrado). Tente novamente." };
        }

        let parsed: any;
        try {
          const rawJson = jsonMatch[0];
          // 1ª tentativa: limpar vírgulas finais e parsear normalmente
          const cleanedJson = rawJson.replace(/,\s*([\]}])/g, '$1').trim();
          try {
            parsed = JSON.parse(cleanedJson);
          } catch {
            // 2ª tentativa: recuperar JSON truncado pelo modelo pequeno
            // Estratégia: encontrar o último objeto completo na array clips[]
            parsed = repairTruncatedOllamaJson(rawJson);
          }
        } catch (parseError: any) {
          console.error("Failed to parse Ollama JSON response:", parseError);
          return { clips: [], error: `O modelo Ollama gerou JSON incompleto. Tente um modelo maior (ex: llama3.2:3b) ou use OpenAI/Lovable API.\n\nDetalhes: ${parseError.message}` };
        }

        if (!parsed.clips || !Array.isArray(parsed.clips)) {
          return { clips: [], error: "O campo principal 'clips' não foi encontrado na resposta do Ollama." };
        }

        // Sanitização de segurança contra chaves em português ou campos em falta
        const sanitizedClips: ViralClip[] = parsed.clips.map((clipObj: any) => {
          ...
          // Calcular duração em segundos no JS
          ...
          if (durationSeconds <= 0 || isNaN(durationSeconds)) {
            durationSeconds = 30; // fallback padrão
          }

          // Extrair o trecho real da transcrição deterministicamente
          const excerpt = extractTranscriptExcerpt(data.transcript, startTimestamp, endTimestamp);

          // Configurar estilos e B-rolls padrão para reduzir o processamento da IA
          ...
          // Garantir que o título e o hook sejam baseados no conteúdo real se a IA alucinar placeholders
          ...
          if (isTitleBad || isHookBad) {
            const engaging = buildEngagingTitleAndHook(excerpt, (clipObj.triggers || ["hook"]) as ClipTrigger[], startTimestamp);
            if (isTitleBad) {
          ...
          return {
            ...
            durationSeconds,
            hookQuote,
            triggers: (Array.isArray(clipObj.triggers) 
              ? clipObj.triggers 
              : Array.isArray(clipObj.gatilhos) 
                ? clipObj.gatilhos 
                : ["hook"]
            ).map((t: any) => {
              const str = String(t).toLowerCase();
              if (["hook", "cliffhanger", "high_value", "controversy", "emotional", "humor"].includes(str)) {
                return str as ClipTrigger;
              }
        ...
        if (shouldGenerateThumbnails && data.videoPath) {
          ...
          // ⚡ Pré-resolver URL de stream do YouTube UMA vez para todos os clips
          let resolvedStreamUrl: string | undefined;
          const isYouTube = data.videoPath.includes("youtube.com") || data.videoPath.includes("youtu.be");
          if (isYouTube) {
            try {
              ...
              if (resolved && resolved.startsWith("http")) {
                resolvedStreamUrl = resolved;
                console.log(`✅ Stream URL resolvida com sucesso! Reutilizando para todos os ${sorted.length} clips.`);
              }
            } catch (ytErr) {
          ...
          const clipsWithThumbs = [] as typeof sorted;
          for (let index = 0; index < sorted.length; index++) {
            const clip = sorted[index];
            try {
              // Usar o trigger REAL do clip para determinar estilo visual único
              const clipRealTrigger = (clip.triggers && clip.triggers[0]) ? clip.triggers[0] : allTriggerTypes[index % allTriggerTypes.length];
              const selectedTrigger = allTriggerTypes.includes(clipRealTrigger as any)
                ? (clipRealTrigger as typeof allTriggerTypes[number])
                : allTriggerTypes[index % allTriggerTypes.length];
              ...
              // Extrair frame em um ponto variado do clip: início+2s, meio do clip, 80% do clip
              const durationSec = clip.durationSeconds || 30;
              const EXTRACT_OFFSETS = [2, Math.floor(durationSec * 0.4), Math.floor(durationSec * 0.75)];
              const extractAtSeconds = startSeconds + EXTRACT_OFFSETS[index % EXTRACT_OFFSETS.length];

              // Background baseado no trigger real do clip (cada trigger tem cor diferente)
              const backgroundTemplate = TRIGGER_BG_MAP[selectedTrigger] ?? "dark_gradient";

              console.log(`  [${index + 1}/${sorted.length}] Trigger real: ${selectedTrigger}, BG: ${backgroundTemplate}, ExtractAt: ${extractAtSeconds}s (Clip: ${clip.startTimestamp} dur:${durationSec}s)`);

              const result = await generateProfessionalThumbnail({
                data: {
                  videoPath: data.videoPath!,
                  resolvedStreamUrl, // ⚡ URL pré-resolvida: evita yt-dlp por thumbnail
                  clipTitle: clip.title,
                  clipHook: clip.hookQuote,
                  triggerType: selectedTrigger,
                  extractAtSeconds,
          ...
          return { clips: ensureDistinctClipSet(clipsWithThumbs, data.transcript, data.videoTitle || "") };
        }
      ...
      const sanitizedClips = parsed.clips.map((clipObj: any) => {
        const id = clipObj.id || `clip_${Math.random().toString(36).substring(2, 11)}_${Date.now()}`;
        ...
        // Extrair excerpt da transcrição
        const excerpt = extractTranscriptExcerpt(data.transcript, startTimestamp, endTimestamp);
      ...
      if (shouldGenerateThumbnails && data.videoPath) {
        ...
        const TRIGGER_BG_MAP2: Record<string, "dark_gradient" | "vibrant_gradient" | "city_night" | "abstract"> = {
          humor:       "vibrant_gradient",
          controversy: "dark_gradient",
          emotional:   "city_night",
          hook:        "dark_gradient",
        ...
        for (let index = 0; index < sorted.length; index++) {
          const clip = sorted[index];
          try {
            // Usar o trigger REAL do clip para estilo visual único
            const clipRealTrigger2 = (clip.triggers && clip.triggers[0]) ? clip.triggers[0] : allTriggerTypes[index % allTriggerTypes.length];
            const selectedTrigger = allTriggerTypes.includes(clipRealTrigger2 as any)
              ? (clipRealTrigger2 as typeof allTriggerTypes[number])
              : allTriggerTypes[index % allTriggerTypes.length];
            ...
            // Extrair frame em ponto variado: início+2s, 40% do clip, 75% do clip
            ...
            // Background baseado no trigger real (garante diversidade por conteúdo)
...
export const saveClipFeedback = createServerFn({ method: "POST" })
  .inputValidator((data: unknown) => SaveFeedbackSchema.parse(data))
  .handler(async ({ data }) => {
    try {
      const admin = workerSupabase as any;
      
      // Procurar se já existe um feedback para este mesmo trecho
      const { data: existing } = await admin
        .from("clip_feedback")
        .select("id")
        .eq("transcript_excerpt", data.transcriptExcerpt)
        .limit(1);
      ...
      return { success: false, error: e.message || "Erro interno do servidor." };
    }
  });
